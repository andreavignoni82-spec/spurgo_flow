...
function renderLogin(app){
  app.append(el(`<div class="login-wrap">
    <section class="login-visual">
      <div class="brand-lockup"><img src="icon.svg" alt="Spurgo Flow"><div class="brand-title">SPURGO <span>FLOW</span></div></div>
      <h2>La webapp per la gestione completa degli interventi di spurgo.</h2>
      <p class="meta" style="max-width:720px;font-size:16px">Agenda, operatori, fotografie, GPS, relazioni tecniche, firma cliente e rapportino professionale in un unico flusso di lavoro.</p>
      <div class="feature-strip"><div class="feature-mini"><b>📍 Lavoro sul campo</b><span class="meta">Navigazione, GPS e foto direttamente dallo smartphone.</span></div><div class="feature-mini"><b>✨ Relazioni smart</b><span class="meta">Dettatura e relazione tecnica professionale.</span></div><div class="feature-mini"><b>📄 Condivisione</b><span class="meta">PDF, Mail e WhatsApp dal rapportino concluso.</span></div></div>
    </section>
    <aside class="login-side"><div class="login-card">
      <div class="brand-lockup" style="gap:12px"><img src="icon.svg" style="width:60px;height:60px;border-radius:16px" alt=""><div><div style="font-weight:950;font-size:24px">SPURGO <span style="color:#49ca7f">FLOW</span></div><div class="meta">ACCESSO SICURO</div></div></div>
      <form id="loginForm" class="login-form"><div class="field"><label>Utente</label><input id="loginUser" autocomplete="username" required placeholder="es. marco"></div><div class="field"><label>Password</label><input id="loginPass" type="password" autocomplete="current-password" required></div><div id="loginError" class="error-text"></div><button class="btn btn-success btn-block btn-large">ACCEDI</button></form>
  ...
  document.getElementById('loginForm').onsubmit=async e=>{e.preventDefault();const username=document.getElementById('loginUser').value.trim().toLowerCase();const hash=await sha256(document.getElementById('loginPass').value);const u=state.db.users.find(x=>x.username.toLowerCase()===username&&x.passwordHash===hash&&x.enabled);if(!u){document.getElementById('loginError').textContent='Utente o password non corretti.';return;}state.session={userId:u.id,loginAt:nowIso()};saveSession();render();};
}
...
function officeChrome(content){
  const u=currentUser();
  return el(`<div class="office-shell"><aside class="sidebar"><div class="side-brand"><img src="icon.svg" alt=""><strong>SPURGO <span>FLOW</span></strong></div><nav class="side-nav"><button class="active" id="navDashboard">⌂ Dashboard</button><button id="navNew">＋ Interventi</button><button id="navAgenda">▣ Agenda</button><button id="navClients">▤ Clienti</button><button id="navOperators">♙ Operatori</button><button id="navSurveys">▥ Sopralluoghi</button><button id="navAnomalies">△ Anomalie</button><button id="navStats">▥ Statistiche</button><button id="navSettings">⚙ Impostazioni</button></nav><div class="side-footer"><div class="side-user"><div class="side-avatar">${esc((u?.name||'U').charAt(0))}</div><div><strong>${esc(u?.name||'Ufficio')}</strong><div style="font-size:10px;color:#aebfd0">UFFICIO</div></div></div><button class="btn btn-secondary btn-block" style="margin-top:12px" id="logoutBtn">Esci</button></div></aside><div class="office-main"><div class="office-topbar"><div class="office-title">Centrale operativa</div><div class="meta">${esc(TODAY_LABEL)}</div></div>${content}</div></div>`);
}

function renderOffice(app){
  const jobs=[...state.db.jobs],counts={unassigned:0,planned:0,progress:0,done:0,urgent:0};jobs.forEach(j=>counts[j.status]=(counts[j.status]||0)+1);const pendingSurveys=jobs.filter(j=>j.survey).length,anomalies=jobs.reduce((n,j)=>n+(j.anomalies?.length||0),0);
  app.append(officeChrome(`<main class="container"><div class="hero"><div><div class="eyebrow">DASHBOARD</div><h1>Buongiorno, Ufficio</h1><p>${esc(getSettings().companyName)} · situazione operativa aggiornata</p></div><div class="hero-actions"><button class="btn btn-secondary" id="manageUsers">👥 Operatori</button><button class="btn btn-primary" id="newJob">＋ Nuovo intervento</button></div></div><div class="grid kpis" id="kpis"></div><div class="grid two" style="margin-top:16px"><section class="card"><div class="section-title"><h2>Interventi di oggi</h2><span class="pill">${jobs.length} totali</span></div><div id="jobList"></div></section><section><div class="card"><div class="section-title"><h2>Mappa interventi</h2><span class="pill">LIVE DEMO</span></div><div class="map-placeholder"><i class="map-pin" style="left:33%;top:45%"></i><i class="map-pin" style="left:67%;top:34%;background:#f59e0b"></i><i class="map-pin" style="left:55%;top:70%;background:#2488e8"></i></div></div><div class="mini-panel"><div class="quick-row"><strong>Sopralluoghi da preventivare</strong><span>${pendingSurveys}</span></div><div class="quick-row"><strong>Anomalie aperte</strong><span>${anomalies}</span></div><div class="quick-row"><strong>Operatori attivi</strong><span>${operatorUsers().length}</span></div></div></section></div><div class="grid two" style="margin-top:16px"><section class="card"><div class="section-title"><h2>Agenda operatori</h2></div><div id="agenda"></div></section><section class="card"><div class="section-title"><h2>Clienti e memoria tecnica</h2></div><div id="clients"></div></section></div></main>`));wireTopbar();
  document.getElementById('kpis').innerHTML=[['Interventi oggi',jobs.length],['Da assegnare',counts.unassigned],['In corso',counts.progress],['Completati',counts.done],['Urgenti',counts.urgent]].map(x=>`<div class="card kpi"><strong>${x[1]}</strong><span>${x[0]}</span></div>`).join('');
  document.getElementById('jobList').innerHTML=jobs.sort((a,b)=>(a.time||'99:99').localeCompare(b.time||'99:99')).map(j=>`<div class="job"><div class="status-bar ${statusClass(j.status)}"></div><div><div class="pill">${statusLabel(j.status)}</div><h3>${esc(j.client)}</h3><div class="meta">${esc(j.time)} · ${esc(j.address)}<br>${esc(j.issue)} · ${esc(operatorName(j.operatorId))}</div></div><button class="btn btn-secondary" data-open-job="${j.id}">Apri</button></div>`).join('');document.querySelectorAll('[data-open-job]').forEach(b=>b.onclick=()=>openOfficeJob(Number(b.dataset.openJob)));
  ...
  const grouped=[...new Map(jobs.map(j=>[j.client,j])).values()].slice(0,5);document.getElementById('clients').innerHTML=grouped.map(j=>`<div class="history-item"><strong>${esc(j.client)}</strong><div class="meta">${esc(j.address)}<br>${esc(j.permanent||'Nessuna memoria tecnica.')}</div><button class="btn btn-secondary" style="margin-top:7px" data-client="${encodeURIComponent(j.client)}">Storico</button></div>`).join('');document.querySelectorAll('[data-client]').forEach(b=>b.onclick=()=>openClient(decodeURIComponent(b.dataset.client)));
  document.getElementById('newJob').onclick=renderNewJob;document.getElementById('manageUsers').onclick=renderUsers;document.getElementById('navDashboard').onclick=render;document.getElementById('navNew').onclick=renderNewJob;document.getElementById('navOperators').onclick=renderUsers;document.getElementById('navSettings').onclick=renderSettings;['navAgenda','navClients','navSurveys','navAnomalies','navStats'].forEach(id=>document.getElementById(id).onclick=()=>alert('Sezione predisposta per la prossima versione. I dati principali sono già disponibili in Dashboard.'));
}

function openOfficeJob(id){
  const j=state.db.jobs.find(x=>x.id===id);if(!j)return render();const app=document.getElementById('app');app.innerHTML='';app.append(topbar());const opts=operatorUsers().map(o=>`<option value="${o.id}">${esc(o.name)}</option>`).join('');
  app.append(el(`<main class="container"><button class="btn btn-secondary" id="backDash">← Dashboard</button><div class="hero" style="margin-top:18px"><div><div class="eyebrow">INTERVENTO #${j.id}</div><h1>${esc(j.client)}</h1><p>${esc(j.address)}</p></div><span class="pill">${statusLabel(j.status)}</span></div><div class="grid two"><section class="card"><h3>${esc(j.issue)}</h3><div class="meta">${esc(j.time)} · ${esc(j.priority)}<br>Operatore: ${esc(operatorName(j.operatorId))} · Mezzo: ${esc(j.vehicle||'—')}</div><div class="section-title"><h2>Note ufficio</h2></div><p>${esc(j.notes||'—')}</p><div class="section-title"><h2>Memoria tecnica</h2></div><div class="note">${esc(j.permanent||'Nessuna memoria tecnica.')}</div>${j.report?`<div class="section-title"><h2>Relazione tecnica</h2></div><div class="note report-note">${esc(j.report).replace(/\n/g,'<br>')}</div>`:''}${j.gps?`<p class="meta">📍 GPS: ${j.gps.lat.toFixed(5)}, ${j.gps.lng.toFixed(5)}</p>`:''}</section><section><div class="card"><h3>Gestione intervento</h3><div class="field"><label>Operatore</label><select id="assignOp"><option value="">Da assegnare</option>${opts}</select></div><div class="field"><label>Mezzo</label><select id="assignVehicle"><option value="">—</option><option>Autobotte 01</option><option>Autobotte 02</option><option>Mezzo compatto</option></select></div><button class="btn btn-primary btn-block" id="saveAssign">Salva assegnazione</button><div class="section-title"><h2>Documentazione</h2></div><div class="photo-grid">${(j.photos||[]).map(p=>`<img src="${p.data}" alt="${esc(p.category)}">`).join('')||'<div class="empty">Nessuna foto</div>'}</div></div>${j.status==='done'?sharePanelHtml(j):'<div class="note" style="margin-top:14px">Il pannello di invio si attiva quando l’intervento viene completato.</div>'}</section></div></main>`));wireTopbar();
  document.getElementById('backDash').onclick=render;document.getElementById('assignOp').value=j.operatorId||'';document.getElementById('assignVehicle').value=j.vehicle||'';document.getElementById('saveAssign').onclick=()=>{j.operatorId=document.getElementById('assignOp').value;j.vehicle=document.getElementById('assignVehicle').value;if(j.status!=='done'&&j.status!=='progress')j.status=j.operatorId?(j.priority==='Emergenza'?'urgent':'planned'):'unassigned';saveDb();openOfficeJob(j.id)};if(j.status==='done')wireSharePanel(j);
}
...
function reportPreviewHtml(j){
  const date=new Intl.DateTimeFormat('it-IT').format(new Date(j.endedAt||Date.now()));
  const start=j.startedAt?new Date(j.startedAt).toLocaleTimeString('it-IT',{hour:'2-digit',minute:'2-digit'}):'—';
  const end=j.endedAt?new Date(j.endedAt).toLocaleTimeString('it-IT',{hour:'2-digit',minute:'2-digit'}):'—';
  const dur=j.startedAt&&j.endedAt?Math.max(1,Math.round((new Date(j.endedAt)-new Date(j.startedAt))/60000))+' min':'—';
  const photos=(j.photos||[]).slice(0,4);
  return `<article class="report-sheet">
    <header class="report-head"><div class="report-brand"><img src="icon.svg" alt=""><div><strong>SPURGO <span>FLOW</span></strong><small>GESTIONE INTERVENTI SPURGHI</small></div></div><div class="report-id"><b>RAPPORTINO INTERVENTO</b><span>N. ${j.id}</span></div><div class="qr-demo" aria-label="QR rapportino">▦</div></header>
    <div class="report-rule"></div>
    <section class="report-cols"><div><h4>DATI CLIENTE</h4><b>${esc(j.client)}</b><p>${esc(j.address)}<br>${j.phone?`Tel. ${esc(j.phone)}`:''}${j.email?`<br>${esc(j.email)}`:''}</p></div><div><h4>DATI INTERVENTO</h4><dl><dt>Data</dt><dd>${date}</dd><dt>Orario inizio</dt><dd>${start}</dd><dt>Orario fine</dt><dd>${end}</dd><dt>Durata</dt><dd>${dur}</dd><dt>Operatore</dt><dd>${esc(operatorName(j.operatorId))}</dd><dt>Mezzo</dt><dd>${esc(j.vehicle||'—')}</dd></dl></div></section>
    <section class="report-section"><h4>RICHIESTA CLIENTE</h4><p>${esc(j.issue)}${j.notes?`<br>${esc(j.notes)}`:''}</p></section>
    <section class="report-split"><div><h4>ATTIVITÀ ESEGUITE</h4>${(j.activities||[]).length?`<ul class="check-list">${j.activities.map(x=>`<li>✓ ${esc(x)}</li>`).join('')}</ul>`:'<p>—</p>'}</div><div class="anomaly-box"><h4>⚠ ANOMALIE RISCONTRATE</h4>${(j.anomalies||[]).length?`<ul>${j.anomalies.map(x=>`<li>${esc(x.text)}</li>`).join('')}</ul>`:'<p>Nessuna anomalia segnalata.</p>'}</div></section>
    <section class="report-section"><h4>RELAZIONE TECNICA</h4><p class="report-copy">${esc(j.report||j.transcript||'Nessuna relazione disponibile.').replace(/\n/g,'<br>')}</p></section>
    <section class="report-section"><h4>DOCUMENTAZIONE FOTOGRAFICA</h4><div class="report-photos">${photos.length?photos.map(p=>`<figure><img src="${p.data}" alt="${esc(p.category)}"><figcaption>${esc(p.category||'FOTO')}</figcaption></figure>`).join(''):'<p class="muted-inline">Nessuna foto allegata</p>'}</div></section>
    <section class="report-bottom"><div class="position-card"><h4>POSIZIONE INTERVENTO</h4><p>${j.gps?`📍 ${j.gps.lat.toFixed(6)}, ${j.gps.lng.toFixed(6)}<br><span>Precisione ±${Math.round(j.gps.accuracy||0)} m</span>`:'Posizione non acquisita'}</p></div><div class="sign-card"><h4>FIRMA CLIENTE</h4><b>${esc(j.signer||'—')}</b>${j.signature?`<img src="${j.signature}" alt="Firma cliente">`:''}</div><div class="sign-card"><h4>FIRMA OPERATORE</h4><b>${esc(operatorName(j.operatorId))}</b><div class="operator-sign">${esc(operatorName(j.operatorId))}</div></div></section>
    <footer class="report-footer">${esc(getSettings().companyName||'Spurgo Flow')} · Rapportino digitale generato con Spurgo Flow</footer>
  </article>`;
}
...
function renderDoneJob(j){
  ...
  app.append(el(`<div class="report-page"><div class="report-toolbar"><button class="btn btn-secondary" id="backToday">← ${isOffice?'Dashboard':'I miei interventi'}</button><div><span class="pill pill-ok">COMPLETATO</span><strong>Rapportino #${j.id}</strong></div><button class="btn btn-primary" id="downloadTop">Scarica PDF</button></div><div class="report-page-grid"><div class="report-preview-wrap">${reportPreviewHtml(j)}</div>${sharePanelHtml(j)}</div></div>`));
  document.getElementById('backToday').onclick=render;
  document.getElementById('downloadTop').onclick=()=>generatePdf(j);
  wireSharePanel(j);
}

function sharePanelHtml(j){return `<aside class="share-panel share-panel-pro"><div class="share-title"><div><div class="eyebrow">INVIA RAPPORTINO</div><h3>Scegli come consegnarlo</h3></div><span class="pill">#${j.id}</span></div><div class="share-recipient"><span>Destinatario</span><b>${esc(j.email||j.phone||'Da scegliere')}</b></div><div class="share-grid"><button class="share-choice blue" id="shareEmail"><span class="share-icon">✉</span><strong>INVIA VIA E-MAIL</strong><small>Prepara e-mail e PDF</small></button><button class="share-choice green" id="shareWhatsApp"><span class="share-icon">◉</span><strong>INVIA VIA WHATSAPP</strong><small>Condividi PDF e messaggio</small></button><button class="share-choice navy" id="sharePdf"><span class="share-icon">↗</span><strong>CONDIVIDI PDF</strong><small>Mail, WhatsApp, AirDrop…</small></button></div><button class="btn btn-secondary btn-block" style="margin-top:12px" id="downloadPdf">📄 Scarica una copia PDF</button><div class="share-help">Su iPhone/Android la condivisione nativa può allegare realmente il PDF. Su browser desktop il sistema scarica il PDF e apre il canale scelto.</div></aside>`;}
function wireSharePanel(j){document.getElementById('downloadPdf')?.addEventListener('click',()=>generatePdf(j));document.getElementById('sharePdf')?.addEventListener('click',()=>sharePdfFile(j));document.getElementById('shareEmail')?.addEventListener('click',()=>shareEmail(j));document.getElementById('shareWhatsApp')?.addEventListener('click',()=>shareWhatsApp(j));}
function shareText(j){return `Buongiorno, in allegato il rapportino dell'intervento n. ${j.id} eseguito presso ${j.client} - ${j.address}.`;}
async function buildPdfBlob(j){
  const jsPDF=window.jspdf?.jsPDF;if(!jsPDF)return null;
  const s=getSettings(),doc=new jsPDF({unit:'mm',format:'a4'}),navy=[6,26,51],green=[49,184,107],line=[195,205,216],textc=[22,34,50],muted=[105,118,135];
  const M=14,W=182; let y=12;
  const date=new Intl.DateTimeFormat('it-IT').format(new Date(j.endedAt||Date.now()));
  const start=j.startedAt?new Date(j.startedAt).toLocaleTimeString('it-IT',{hour:'2-digit',minute:'2-digit'}):'—';
  const end=j.endedAt?new Date(j.endedAt).toLocaleTimeString('it-IT',{hour:'2-digit',minute:'2-digit'}):'—';
  const dur=j.startedAt&&j.endedAt?Math.max(1,Math.round((new Date(j.endedAt)-new Date(j.startedAt))/60000))+' min':'—';
  const section=(title,yy)=>{doc.setFont('helvetica','bold');doc.setFontSize(7.4);doc.setTextColor(...navy);doc.text(title,M,yy);doc.setDrawColor(...line);doc.line(M,yy+2,196,yy+2);};
  const wrap=(txt,x,yy,w,size=8)=>{doc.setFont('helvetica','normal');doc.setFontSize(size);doc.setTextColor(...textc);const ls=doc.splitTextToSize(String(txt||'—'),w);doc.text(ls,x,yy);return yy+ls.length*3.6;};
  doc.setFillColor(...navy);doc.roundedRect(M,y,182,21,2,2,'F');
  doc.setFont('helvetica','bold');doc.setFontSize(16);doc.setTextColor(255);doc.text('SPURGO',M+5,y+10);doc.setTextColor(...green);doc.text('FLOW',M+34,y+10);doc.setTextColor(255);doc.setFontSize(6);doc.text('GESTIONE INTERVENTI SPURGHI',M+5,y+16);
  doc.setFontSize(9);doc.text('RAPPORTINO INTERVENTO',128,y+8);doc.setFontSize(8);doc.text(`N. ${j.id}`,128,y+14);y+=29;
  section('DATI CLIENTE',y); doc.setFontSize(8);doc.setFont('helvetica','bold');doc.setTextColor(...textc);doc.text(j.client,M,y+8); doc.setFont('helvetica','normal');doc.text(doc.splitTextToSize(j.address||'—',80),M,y+13);if(j.phone)doc.text(`Tel. ${j.phone}`,M,y+22);
  doc.setDrawColor(...line);doc.line(108,y+4,108,y+31);doc.setFont('helvetica','bold');doc.setTextColor(...navy);doc.text('DATI INTERVENTO',114,y+8);doc.setFont('helvetica','normal');doc.setTextColor(...textc);doc.text([`Data: ${date}`,`Orario inizio: ${start}`,`Orario fine: ${end}`,`Durata: ${dur}`,`Operatore: ${operatorName(j.operatorId)}`,`Mezzo: ${j.vehicle||'—'}`],114,y+13);y+=39;
  section('RICHIESTA CLIENTE',y);y=wrap(j.issue+(j.notes?`\n${j.notes}`:''),M,y+8,W,8)+4;
  section('ATTIVITÀ ESEGUITE',y);const acts=(j.activities||[]).length?j.activities.map(x=>`✓ ${x}`).join('\n'):'—';y=wrap(acts,M,y+8,85,8);const anY=y;doc.setFont('helvetica','bold');doc.setTextColor(...navy);doc.text('ANOMALIE RISCONTRATE',112,anY);const ans=(j.anomalies||[]).length?j.anomalies.map(x=>`• ${x.text}`).join('\n'):'Nessuna anomalia segnalata.';wrap(ans,112,anY+5,82,8);y=Math.max(y,anY+18)+3;
  section('RELAZIONE TECNICA',y);y=wrap(j.report||j.transcript||'Nessuna relazione disponibile.',M,y+8,W,8)+4;
  if(y>205){doc.addPage();y=16;}
  section('DOCUMENTAZIONE FOTOGRAFICA',y);y+=7;let x=M;for(const p of (j.photos||[]).slice(0,4)){try{doc.addImage(p.data,'JPEG',x,y,40,27);doc.setFontSize(6);doc.setTextColor(...muted);doc.text(p.category||'FOTO',x,y+31);x+=46;}catch{}}y+=37;
  doc.setDrawColor(...line);doc.roundedRect(M,y,58,30,1.5,1.5);doc.roundedRect(76,y,56,30,1.5,1.5);doc.roundedRect(136,y,60,30,1.5,1.5);
  doc.setFont('helvetica','bold');doc.setFontSize(6.8);doc.setTextColor(...navy);doc.text('POSIZIONE INTERVENTO',M+3,y+5);doc.text('FIRMA CLIENTE',79,y+5);doc.text('FIRMA OPERATORE',139,y+5);
  doc.setFont('helvetica','normal');doc.setFontSize(7);doc.setTextColor(...textc);doc.text(j.gps?doc.splitTextToSize(`${j.gps.lat.toFixed(5)}, ${j.gps.lng.toFixed(5)}`,50):'Non acquisita',M+3,y+11);doc.text(j.signer||'—',79,y+11);doc.text(operatorName(j.operatorId),139,y+11);
  if(j.signature){try{doc.addImage(j.signature,'PNG',79,y+13,43,13)}catch{}}
  doc.setFillColor(...navy);doc.rect(0,286,210,11,'F');doc.setTextColor(255);doc.setFontSize(6.5);doc.text(`${s.companyName||'Spurgo Flow'} · Rapportino digitale generato con Spurgo Flow`,M,293);
  return doc.output('blob');
}
async function generatePdf(j){const blob=await buildPdfBlob(j);if(!blob)return printReport(j);const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`Rapportino_${j.id}_${safeFile(j.client)}.pdf`;a.click();setTimeout(()=>URL.revokeObjectURL(url),2000);}
async function sharePdfFile(j){const blob=await buildPdfBlob(j);if(!blob)return generatePdf(j);const file=new File([blob],`Rapportino_${j.id}_${safeFile(j.client)}.pdf`,{type:'application/pdf'});if(navigator.share&&(!navigator.canShare||navigator.canShare({files:[file]}))){try{await navigator.share({title:`Rapportino intervento #${j.id}`,text:shareText(j),files:[file]});return}catch(e){if(e.name==='AbortError')return}}await generatePdf(j);alert('PDF scaricato. La condivisione diretta dei file non è disponibile su questo dispositivo/browser.');}
async function shareEmail(j){
  const subject=encodeURIComponent(`Rapportino intervento #${j.id} - ${j.client}`),body=encodeURIComponent(shareText(j)+`

Cordiali saluti,
${getSettings().companyName||'Spurgo Flow'}`);
  const blob=await buildPdfBlob(j);const file=blob?new File([blob],`Rapportino_${j.id}_${safeFile(j.client)}.pdf`,{type:'application/pdf'}):null;
  if(file&&navigator.share&&(!navigator.canShare||navigator.canShare({files:[file]}))){try{await navigator.share({title:`Rapportino intervento #${j.id}`,text:shareText(j),files:[file]});return}catch(e){if(e.name==='AbortError')return}}
  if(blob) await generatePdf(j); location.href=`mailto:${encodeURIComponent(j.email||'')}?subject=${subject}&body=${body}`;
}
async function shareWhatsApp(j){
  const txt=encodeURIComponent(shareText(j));const blob=await buildPdfBlob(j);const file=blob?new File([blob],`Rapportino_${j.id}_${safeFile(j.client)}.pdf`,{type:'application/pdf'}):null;
  if(file&&navigator.share&&(!navigator.canShare||navigator.canShare({files:[file]}))){try{await navigator.share({title:`Rapportino #${j.id}`,text:shareText(j),files:[file]});return}catch(e){if(e.name==='AbortError')return}}
  if(blob) await generatePdf(j);window.open(`https://wa.me/${String(j.phone||'').replace(/\D/g,'')}?text=${txt}`,'_blank');
}

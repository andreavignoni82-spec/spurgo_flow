...
function renderDoneJob(j){
  const app=document.getElementById('app');app.innerHTML='';app.append(el(`<div class="mobile-shell"><div class="mobile-header"><button class="btn btn-secondary" id="backToday">← Oggi</button><div class="pill pill-ok">COMPLETATO</div><h1>${esc(j.client)}</h1><div>${esc(j.issue)}</div></div><div class="mobile-content"><div class="card"><div class="section-title"><h2>Rapportino pronto</h2><span class="pill">#${j.id}</span></div><div class="note report-note">${esc(j.report||'').replace(/\n/g,'<br>')}</div><p><strong>Firmatario:</strong> ${esc(j.signer||'—')}</p>${j.signature?`<img class="signature-image" src="${j.signature}" alt="Firma cliente">`:''}<div class="photo-grid">${(j.photos||[]).map(p=>`<img src="${p.data}" alt="${esc(p.category)}">`).join('')}</div></div>${sharePanelHtml(j)}</div></div>`));document.getElementById('backToday').onclick=render;wireSharePanel(j);
}

function sharePanelHtml(j){return `<div class="share-panel"><div class="eyebrow">INVIA RAPPORTINO</div><h3>Condividi il PDF dell'intervento</h3><div class="meta">Su iPhone/Android il tasto PDF apre il menu nativo, dal quale puoi scegliere Mail o WhatsApp con il file allegato.</div><div class="share-grid"><button class="share-choice blue" id="shareEmail"><span class="share-icon">✉</span><strong>E-mail</strong><small>Prepara messaggio al cliente</small></button><button class="share-choice green" id="shareWhatsApp"><span class="share-icon">●</span><strong>WhatsApp</strong><small>Prepara messaggio rapido</small></button><button class="share-choice navy" id="sharePdf"><span class="share-icon">↗</span><strong>Condividi PDF</strong><small>Mail, WhatsApp, AirDrop…</small></button></div><button class="btn btn-secondary btn-block" style="margin-top:10px" id="downloadPdf">📄 Scarica una copia PDF</button></div>`;}
...
async function buildPdfBlob(j){
  const jsPDF=window.jspdf?.jsPDF;if(!jsPDF)return null;const s=getSettings(),doc=new jsPDF({unit:'mm',format:'a4'}),navy=[6,26,51],green=[49,184,107],muted=[104,120,139],margin=14,contentW=182;let y=0;const ensure=(n=20)=>{if(y+n>280){doc.addPage();header(false)}};const header=(first=true)=>{doc.setFillColor(...navy);doc.rect(0,0,210,24,'F');doc.setTextColor(255);doc.setFont('helvetica','bold');doc.setFontSize(18);doc.text('SPURGO',margin,15);doc.setTextColor(...green);doc.text('FLOW',margin+31,15);doc.setTextColor(255);doc.setFontSize(9);doc.text(`RAPPORTINO INTERVENTO  #${j.id}`,128,14);doc.setTextColor(15,35,61);y=first?34:31};const line=()=>{doc.setDrawColor(220,228,236);doc.line(margin,y,196,y);y+=5};const title=t=>{ensure(12);doc.setTextColor(...navy);doc.setFont('helvetica','bold');doc.setFontSize(9);doc.text(t.toUpperCase(),margin,y);y+=5};const text=(t,size=9,bold=false,max=contentW)=>{ensure(14);doc.setFont('helvetica',bold?'bold':'normal');doc.setFontSize(size);doc.setTextColor(...(bold?navy:[25,40,58]));const lines=doc.splitTextToSize(String(t||'—'),max);doc.text(lines,margin,y);y+=lines.length*(size*.39)+2};
  header(true);doc.setFont('helvetica','bold');doc.setFontSize(13);doc.setTextColor(...navy);doc.text(s.companyName||'Demo Spurghi Srl',margin,y);y+=7;doc.setFontSize(8);doc.setFont('helvetica','normal');doc.setTextColor(...muted);doc.text([s.companyPhone||'',s.companyEmail||''].filter(Boolean).join('  |  '),margin,y);y+=7;line();title('Dati cliente e intervento');doc.setFillColor(247,249,251);doc.roundedRect(margin,y,contentW,30,2,2,'F');doc.setTextColor(...navy);doc.setFontSize(9);doc.setFont('helvetica','bold');doc.text(j.client,margin+4,y+7);doc.setFont('helvetica','normal');doc.text(j.address,margin+4,y+13);doc.text(`Tel. ${j.phone||'—'}  |  E-mail ${j.email||'—'}`,margin+4,y+19);doc.text(`Data ${new Intl.DateTimeFormat('it-IT').format(new Date(j.endedAt||Date.now()))}  |  Operatore ${operatorName(j.operatorId)}  |  Mezzo ${j.vehicle||'—'}`,margin+4,y+25);y+=36;title('Richiesta cliente');text(j.issue+(j.notes?`\n${j.notes}`:''));line();if(j.activities?.length){title('Attività eseguite');text(j.activities.map(x=>`• ${x}`).join('\n'));}if(j.anomalies?.length){title('Anomalie riscontrate');text(j.anomalies.map(x=>`• ${x.text}`).join('\n'));}line();title('Relazione tecnica');text(j.report||j.transcript||'Nessuna relazione disponibile.');line();if(j.gps){title('Posizione intervento');text(`${j.gps.lat.toFixed(6)}, ${j.gps.lng.toFixed(6)} · precisione ±${Math.round(j.gps.accuracy||0)} m`,8);}if(j.photos?.length){title('Documentazione fotografica');let x=margin;for(const p of j.photos.slice(0,4)){ensure(48);try{doc.addImage(p.data,'JPEG',x,y,40,30);doc.setFontSize(7);doc.setTextColor(...muted);doc.text(p.category||'FOTO',x,y+34);x+=46;if(x>160){x=margin;y+=39}}catch{}}if(x!==margin)y+=39;}ensure(45);line();title('Conferma intervento');doc.setFontSize(8);doc.setTextColor(...navy);doc.text(`Firma cliente: ${j.signer||'—'}`,margin,y);if(j.signature){try{doc.addImage(j.signature,'PNG',margin,y+3,58,24)}catch{}}doc.text(`Operatore: ${operatorName(j.operatorId)}`,120,y);doc.setFillColor(...navy);doc.rect(0,286,210,11,'F');doc.setTextColor(255);doc.setFontSize(7);doc.text('Documento generato da Spurgo Flow · Gestione interventi spurghi',margin,293);return doc.output('blob');
}
...
async function shareEmail(j){const subject=encodeURIComponent(`Rapportino intervento #${j.id} - ${j.client}`),body=encodeURIComponent(shareText(j)+`\n\nCordiali saluti,\n${getSettings().companyName}`);if(navigator.share){const blob=await buildPdfBlob(j);if(blob){const f=new File([blob],`Rapportino_${j.id}_${safeFile(j.client)}.pdf`,{type:'application/pdf'});try{await navigator.share({title:`Rapportino intervento #${j.id}`,text:shareText(j),files:[f]});return}catch(e){if(e.name==='AbortError')return}}}location.href=`mailto:${encodeURIComponent(j.email||'')}?subject=${subject}&body=${body}`;}
async function shareWhatsApp(j){const txt=encodeURIComponent(shareText(j));if(navigator.share){const blob=await buildPdfBlob(j);if(blob){const f=new File([blob],`Rapportino_${j.id}_${safeFile(j.client)}.pdf`,{type:'application/pdf'});try{await navigator.share({title:`Rapportino #${j.id}`,text:shareText(j),files:[f]});return}catch(e){if(e.name==='AbortError')return}}}window.open(`https://wa.me/${String(j.phone||'').replace(/\D/g,'')}?text=${txt}`,'_blank');}

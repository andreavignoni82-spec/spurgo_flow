...
const validFirebase = Object.freeze({ apiKey: 'demo-test-only', authDomain: 'localhost', projectId: 'spurgoflow-test', host: '127.0.0.1', firestorePort: 8080, authPort: 9099, useEmulator: true });
...
test('driver-aware configuration matrix preserves the Firebase project guard', () => {
  ...
  assert.throws(() => createEnvironment({ dataDriver: 'firebase' }), error => error instanceof ConfigurationError && error.code === 'BOOT_CONFIG_FIREBASE_MISSING', 'D: missing config');
  assert.throws(() => createEnvironment({ dataDriver: 'firebase', firebase: { ...validFirebase, projectId: 'unauthorized' } }), error => error instanceof ConfigurationError && error.code === 'BOOT_FIREBASE_PROJECT_BLOCKED', 'E: project guard');
});

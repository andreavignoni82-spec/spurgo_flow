...
assert.match(visibleText, /v8\.0\.0-alpha\.2\.1/);
...
assert.ok((await page.evaluate(() => caches.keys())).includes('spurgoflow-v8-0.0-alpha.2.1-data-core-hardening'), 'alpha.2.1 service-worker cache must exist');
assert.deepEqual(fatalErrors, []);
await mkdir('artifacts', { recursive: true });
await page.screenshot({ path: 'artifacts/spurgo-flow-v8-alpha2.1.png', fullPage: true });
await browser.close();

...
export function createFirebaseClient(config={}){
  ...
  const adapter={
    ...
    async replace(name,id,value){await setDoc(doc(firestore,name,id),value);const snapshot=await getDoc(doc(firestore,name,id));return row(snapshot)},
  ...
  const deleteProfile=async uid=>{const reference=doc(firestore,'profiles',uid);const snapshot=await getDoc(reference);if(snapshot.exists())await deleteDoc(reference);return {uid,accessRevoked:true};};
  ...
  return Object.freeze({app,auth,firestore,adapter,loadProfile,saveProfile,deleteProfile,provisionUser,config:Object.freeze({...config}),close:()=>deleteApp(app)});
}

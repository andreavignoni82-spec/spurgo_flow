...
const failure = (error, operation, idField, id) => {
  if (error instanceof RepositoryError) return error;
  if (error?.code === 'already-exists') return new RepositoryError('DUPLICATE_ID', `Duplicate ${idField}: ${id}`, { cause: error });
  if (error?.code === 'not-found') return new RepositoryError('MISSING_ID', `Missing ${idField}: ${id}`, { cause: error });
  const mapped = mapRepositoryError(error); mapped.operation = operation; return mapped;
};
export class FirebaseRepositoryAdapter {
  constructor({ collection, idField = 'id', client, normalize = clone, validate = () => true }) { if (!client) throw new Error('Firebase test client is required'); Object.assign(this, { collection, idField, client, normalize, validate }); }
...
export function createFirebaseRepositories({ client }) {
  ...
  interventions.queryByDate = date => interventions.list().then(items => items.filter(item => item.date === date));
  interventions.queryByOperator = id => interventions.list().then(items => items.filter(item => item.operatorId === id || item.assignedOperatorIds.includes(id)));
  interventions.queryByTeam = id => interventions.list().then(items => items.filter(item => item.teamId === id || item.assignedTeamIds.includes(id)));
  ...
  return { clients:make('clients'), operators:make('operators'), teams:make('teams'), vehicles:make('vehicles'), interventions, reports, messages:make('messages') };
}

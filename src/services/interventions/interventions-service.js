...
export class InterventionsService{
 ...
 async assignTeams(id,assignedTeamIds){const ids=[...new Set(assignedTeamIds??[])],teams=await Promise.all(ids.map(teamId=>this.teamsRepository?.getById?.(teamId))),current=await this.repository.getById(id),operators=[...new Set([...(current?.assignedOperatorIds??[]),...teams.flatMap(team=>team?.operatorIds??[])])];return this.#assignment(id,{teamId:ids[0]??null,assignedTeamIds:ids,assignedOperatorIds:operators})}

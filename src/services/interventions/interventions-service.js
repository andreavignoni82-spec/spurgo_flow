...
const CREATE_FIELDS=Object.freeze(['clientId','clientSnapshot','address','city','coordinates','date','startTime','estimatedMinutes','type','priority','description','notes','timeMode','flexibleStartTime','flexibleEndTime']);
...
export class InterventionsService{
 ...
 async saveOfficeIntervention(id,{patch={},operatorIds=[],teamIds=[],vehicleId=''}={}){
   ...
   const teams=await Promise.all(uniq(teamIds).map(teamId=>this.teamsRepository?.getById?.(teamId)));
   const direct=uniq(operatorIds),fromTeams=teams.flatMap(team=>team?.operatorIds??[]),assignedOperatorIds=uniq([...direct,...fromTeams]),assignedTeamIds=uniq(teamIds),timestamp=this.clock();
   const value=await this.repository.update(id,{...patch,operatorId:assignedOperatorIds[0]??null,assignedOperatorIds,teamId:assignedTeamIds[0]??null,assignedTeamIds,vehicleId:vehicleId||null,updatedAt:timestamp});
   this.eventBus?.emit(Events.INTERVENTION_UPDATED,{entityId:id,source:this.source,timestamp});
   this.eventBus?.emit(Events.INTERVENTION_ASSIGNMENT_CHANGED,{entityId:id,source:this.source,timestamp,changes:{operatorId:value.operatorId,assignedOperatorIds,teamId:value.teamId,assignedTeamIds,vehicleId:value.vehicleId}});
   return value;
 }
 async removeIntervention(id){const existing=await this.repository.getById(id);if(!existing)throw new Error(`Missing intervention: ${id}`);const removed=await this.repository.remove(id);this.eventBus?.emit(Events.INTERVENTION_DELETED,{entityId:id,source:this.source,timestamp:this.clock()});return removed}
 assignOperator(id,operatorId){return this.#assignment(id,{operatorId,assignedOperatorIds:operatorId?[operatorId]:[]})}
 assignOperators(id,assignedOperatorIds){const ids=uniq(assignedOperatorIds);return this.#assignment(id,{operatorId:ids[0]??null,assignedOperatorIds:ids})}
 async assignTeam(id,teamId){if(!teamId)return this.#assignment(id,{teamId:null,assignedTeamIds:[]});const team=await this.teamsRepository?.getById?.(teamId),current=await this.repository.getById(id),operators=uniq([...(current?.assignedOperatorIds??[]),...(team?.operatorIds??[])]);return this.#assignment(id,{teamId,assignedTeamIds:[teamId],operatorId:operators[0]??null,assignedOperatorIds:operators})}
 async assignTeams(id,assignedTeamIds){const ids=uniq(assignedTeamIds),teams=await Promise.all(ids.map(teamId=>this.teamsRepository?.getById?.(teamId))),current=await this.repository.getById(id),operators=uniq([...(current?.assignedOperatorIds??[]),...teams.flatMap(team=>team?.operatorIds??[])]);return this.#assignment(id,{teamId:ids[0]??null,assignedTeamIds:ids,operatorId:operators[0]??null,assignedOperatorIds:operators})}
 assignVehicle(id,vehicleId){return this.#assignment(id,{vehicleId:vehicleId||null})}
 ...
 async listForOperator(operatorId,teamIds=[],aliases=[]){const ids=uniq([operatorId,...aliases]);const directRows=[];for(const id of ids){try{const rows=this.repository.queryByOperator?await this.repository.queryByOperator(id):(await this.repository.list()).filter(x=>x.operatorId===id||x.assignedOperatorIds?.includes(id));directRows.push(...rows)}catch{}}
   const teamRows=(await Promise.all(uniq(teamIds).map(id=>this.repository.queryByTeam?this.repository.queryByTeam(id):Promise.resolve([])))).flat();
   let all=[...directRows,...teamRows];
   try{const fallback=await this.repository.list();const teams=uniq(teamIds);all.push(...fallback.filter(x=>ids.includes(String(x.operatorId??''))||(x.assignedOperatorIds??[]).some(id=>ids.includes(String(id)))||teams.includes(String(x.teamId??''))||(x.assignedTeamIds??[]).some(id=>teams.includes(String(id)))))}catch{}

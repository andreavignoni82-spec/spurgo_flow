...
export class InterventionsService {
  ...
  async startIntervention(id) { id=requiredId(id); const current=await this.getIntervention(id); if(!current)throw new Error('Intervention not found');if(!this.status.canTransition(current.status,'In corso'))throw new Error('Invalid intervention status transition');const at=this.now(); const saved=await this.repository.update(id,{status:'In corso',startedAt:at,actualStart:at}); this.#emit('intervention:statusChanged',id,'statusChanged',{previousStatus:current.status,status:'In corso'}); this.#emit('intervention:started',id,'started'); return saved; }
  async completeIntervention(id,data={}) { id=requiredId(id);const current=await this.getIntervention(id);if(!current)throw new Error('Intervention not found');if(!this.status.canTransition(current.status,'Terminato'))throw new Error('Invalid intervention status transition');const at=this.now(); const end=data.endedAt??data.actualEnd??at;const patch={...data,status:'Terminato',endedAt:end,actualEnd:end};const saved=await this.repository.update(id,patch); this.#emit('intervention:statusChanged',id,'statusChanged',{previousStatus:current.status,status:'Terminato'}); this.#emit('intervention:completed',id,'completed'); return saved; }

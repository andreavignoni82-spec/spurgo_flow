...
export function createPlaceholderFeature(id, title) {
  ...
  return {
    ...
    mount(container, { signal }) {
      ...
      element.innerHTML = `<header><strong>SPURGO FLOW</strong><p class="app-shell__eyebrow">${PRODUCT_LABEL}</p></header><nav aria-label="Funzionalità">${navigation}</nav><div class="app-shell__card"><h1>${title}</h1><p>Foundation v8 avviata correttamente</p><p>${VERSION}</p><small>${BUILD_LABEL}</small></div>`;

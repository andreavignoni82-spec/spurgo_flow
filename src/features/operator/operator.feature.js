...
export class OperatorFeature {
  id = 'operator'; #off = []; #generation = 0; #completing = false;
  async mount(container, context = {}) {
    ...
    this.#off = EVENTS.map(name => context.eventBus?.on(name, () => void this.refresh())).filter(Boolean);
  ...
  async saveReport(data) { try { await this.reportController.save(this.currentId, await this.reportController.collect(data)); } catch (error) { if (this.#alive() && error.name !== 'AbortError') this.#message(error.message); } }
  async complete(data) { if (this.#completing || this.reportController.busy) return; this.#completing = true; try { await this.reportController.complete(this.currentId, await this.reportController.collect(data)); } catch (error) { if (error.name === 'AbortError') return; } finally { this.#completing = false; } }

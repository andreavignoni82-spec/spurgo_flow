...
Alpha.2 connects to no production Firebase project and commits no real API key, endpoint, or secret. Memory is the default driver; Firebase is test/emulator-only and opt-in through injected configuration. Authentication is a separate future boundary. Passwords are never accepted or persisted in the Operator domain model.
